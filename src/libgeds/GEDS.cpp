...
GEDS::GEDS(GEDSConfig &&argConfig)
    : std::enable_shared_from_this<GEDS>(), _config(argConfig),
      _server(_config.listenAddress, _config.port), _metadataService(_config.metadataServiceAddress),
      _pathPrefix(_config.localStoragePath), _hostname(_config.hostname.value_or("")),
      _httpServer(_config.portHttpServer) {
  std::error_code ec;
