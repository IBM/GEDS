...
impl RunCommand {
    ...
    pub fn execute(mut self) -> Result<()> {
        ...
        if store.data().preview2_ctx.is_some() {
            ...
            store.data().preview2_ctx.as_ref().unwrap().lock().unwrap().stop_geds();
            let end = SystemTime::now();
            println!("GEDS STOP TIME: {} ms", end.duration_since(start).unwrap().as_millis());
    ...
    fn set_preview2_ctx(&self, store: &mut Store<Host>) -> Result<()> {
        ...
        let enable_geds = self.run.common.wasi.enable_geds.unwrap_or_else(|| false);
        let geds_subscribe = self.run.common.wasi.geds_subscribe.unwrap_or_else(|| false);
        builder.enable_geds(enable_geds);
        builder.geds_subscribe(geds_subscribe);
