...
use crate::filesystem::{Descriptor, Dir, File, FileInputStream, FileOutputStream, OpenMode, ReaddirIterator};
    ...
    T: WasiView,
{
    ...
    async fn sync_data(&mut self, fd: Resource<types::Descriptor>) -> FsResult<()> {
        ...
        match descriptor {
            ...
            Descriptor::GEDSFile(_) => {Err(ErrorCode::BadDescriptor.into())}
        }
    }
...
async fn get_descriptor_metadata(fd: &types::Descriptor) -> FsResult<cap_std::fs::Metadata> {
    match fd {
        ...
        Descriptor::GEDSFile(_) => {Err(types::ErrorCode::BadDescriptor.into())}
    }
}
