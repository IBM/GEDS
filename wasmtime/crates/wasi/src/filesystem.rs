...
use geds_rs::GEDSFile as GEDSFileRs;
use std::io;
use std::mem;
use std::sync::Arc;
...
impl Descriptor {
    pub fn file(&self) -> Result<&File, types::ErrorCode> {
        match self {
            ...
            Descriptor::GEDSFile(_) => Err(types::ErrorCode::BadDescriptor),
        }
    }
    ...
    pub fn dir(&self) -> Result<&Dir, types::ErrorCode> {
        match self {
            ...
            Descriptor::GEDSFile(_) => Err(types::ErrorCode::NotDirectory),
        }
    }

    pub fn geds_file(&self) -> Result<&GEDSFile, types::ErrorCode> {
        match self {
            Descriptor::GEDSFile(f) => Ok(f),
            Descriptor::File(_) => Err(types::ErrorCode::BadDescriptor),
            Descriptor::Dir(_) => Err(types::ErrorCode::BadDescriptor),
        }
    }
    ...
    pub fn is_geds_file(&self) -> bool {
        match self {
            Descriptor::File(_) => false,
            Descriptor::Dir(_) => false,
            Descriptor::GEDSFile(_) => true,
        }
    }
}
...
pub struct GEDSFile {
    pub geds_file: Arc<GEDSFileRs>,
}

impl GEDSFile {
    pub fn new(file: GEDSFileRs) -> GEDSFile {
        GEDSFile {
            geds_file: Arc::new(file)
        }
    }
}
