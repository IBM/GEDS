...
pub struct WasiP1Ctx {
    ...
    geds_contains_prefix: String,
    adapter: WasiPreview1Adapter,
}

impl WasiP1Ctx {
    pub(crate) fn new(wasi: WasiCtx) -> Self {
        println!("Use geds = {}", wasi.use_geds);
        let geds: Option<GEDS> = match wasi.use_geds {
            true => {
                ...
                if let Err(e) = geds.start() {
                    // todo: use proper logging (tracing ?)
                    println!(
                    ...
                    if let Err(e) = geds.register_object_store_config(
                        ...
                        println!("Error registering GEDS: {}", e);
                    } else {
                        println!("GEDS S3 config registered");
                    }

                    if wasi.geds_subscribe {
                        if let Err(e) = geds.subscribe("geds-default", "unused", &1) {
                            println!("Error subscribing to GEDS bucket: {}", e);
                        }
                    }
                ...
                println!("GEDS INIT TIME: {} ms", end.duration_since(start).unwrap().as_millis());
                ret
            }
            false => None,
        };

        // todo replace bucket with parameter, same with prefix

        let ctx = Self {
            table: ResourceTable::new(),
            wasi,
            geds,
            bucket_prefix: "geds-default".to_string(),
            geds_contains_prefix: ".session".to_string(),
...
impl Transaction<'_> {
    ...
    fn get_fd(&self, fd: types::Fd) -> Result<Resource<filesystem::Descriptor>> {
        match self.get_descriptor(fd)? {
            ...
            Descriptor::GEDSFile(GEDSFile { fd, .. }) => Ok(fd.borrowed()),
            Descriptor::Stdin { .. } | Descriptor::Stdout { .. } | Descriptor::Stderr { .. } => {
...
impl WasiP1Ctx {
    pub fn stop_geds(&mut self) {
        if self.geds.is_some() {
            let geds_ref = self.geds.as_ref().unwrap();
            geds_ref.relocate(true);
            let _ = geds_ref.stop();
        }
    }
    ...
    async fn fd_write_impl(
        ...
        match desc {
            ...
            Descriptor::GEDSFile(GEDSFile { fd, .. }) => {
                let fd = fd.borrowed();
                drop(t);
                let geds_file = self.table().get(&fd)?.geds_file()?;

                let buf = first_non_empty_ciovec(memory, ciovs)?;
                let buf = memory.to_vec(buf)?;

                match geds_file.geds_file.write(&buf, 0, buf.len()) {
...
impl wasi_snapshot_preview1::WasiSnapshotPreview1 for WasiP1Ctx {
    ...
    async fn fd_close(
        ...
        match desc {
            ...
            Descriptor::GEDSFile(GEDSFile { fd, .. }) => {
                ...
                if let Err(e) =  geds_file.geds_file.seal() {
                    println!("Error sealing file: {}", e);
                }
    ...
    async fn fd_fdstat_get(
        ...
        let (fd, blocking, append) = match self.transact()?.get_descriptor(fd)? {
            ...
            Descriptor::GEDSFile(GEDSFile { .. }) => {
                ...
                let fs_rights_base = types::Rights::all();
                fs_flags |= types::Fdflags::APPEND;

                return Ok(types::Fdstat {
                    fs_filetype: types::Filetype::RegularFile,
                    fs_flags,
                    fs_rights_base,
                    fs_rights_inheriting: fs_rights_base,
                });
    ...
    async fn fd_filestat_get(
        ...
        match desc {
            ...
            Descriptor::GEDSFile(GEDSFile { .. }) => Err(types::Errno::Badf.into()),
        }
    }
    ...
    async fn fd_read(
        ...
        match desc {
            ...
            Descriptor::GEDSFile(GEDSFile { fd, .. }) => {
                ...
                match geds_file.geds_file.read(&mut buf, 0, len) {
    ...
    fn fd_prestat_get(
        ...
        // todo: should not require modification
    ...
    async fn path_filestat_get(
        ...
        let dirfd = self.get_dir_fd(dirfd)?;
        let path = read_string(memory, path)?;
        if path.contains(&self.geds_contains_prefix) && self.geds.is_some() {
            let status_response = self
                .geds
                .as_ref()
                .unwrap()
                .status(self.bucket_prefix.as_str(), path.as_str());

            if let Ok(status) = status_response {
                Ok(types::Filestat {
                    dev: 1,
                    ino: 8,
                    filetype: match status.is_directory {
                        true => types::Filetype::Directory,
                        false => types::Filetype::RegularFile,
                    },
                    nlink: 0,
                    size: status.size.try_into()?,
                    atim: 1,
                    mtim: 1,
                    ctim: 1,
                })
    ...
    async fn path_open(
        ...
        if path.contains(&self.geds_contains_prefix) && self.geds.is_some() {
            ...
            if oflags.contains(types::Oflags::CREAT) {
                result = self.geds.as_ref().unwrap().create(self.bucket_prefix.as_str(), &path, true);
            } else {
                result = self.geds.as_ref().unwrap().open(self.bucket_prefix.as_str(), &path);
            }

            if let Err(e) = result {
                println!("Could not open GEDSFile: {}", e);
                return Err(types::Errno::Noent.into());
            }

            let file = result.unwrap();
            let t = self.transact()?;
            fd = t.view.table.push(filesystem::Descriptor::GEDSFile(
                crate::filesystem::GEDSFile::new(file),
            ))?;
            drop(t);
        } else {
        ...
        let desc = match t.view.table().get(&fd)? {
            ...
            crate::filesystem::Descriptor::GEDSFile(_) => Descriptor::GEDSFile(GEDSFile { fd }),
        };
    ...
    async fn path_unlink_file(
        ...
        if path.contains(&self.geds_contains_prefix) && !self.geds.is_some() {
            match self
                ...
                .as_ref()
                .unwrap()
                .delete_object(&self.bucket_prefix, &path)
            {
                Ok(()) => (),
                Err(e) => {
